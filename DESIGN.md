# DESIGN.md: Awesome MBSE

## Product

**Name:** Awesome MBSE  
**Purpose:** Public-domain curated index of Model-Based Systems Engineering: SysML v1/v2, methods, tools, and openable models, with the deepest Magic Grid / Cameo / CATIA Magic coverage in the family.  
**Primary users:** MBSE and SysML practitioners.  
...
**Surface:** One GitHub Pages landing (`docs/index.html` path family) with in-page anchors. README on GitHub stays the canonical deep list unless a later pass mirrors entries under this contract without drift. Hub of the awesome-mbse list family (registry in FAMILY.md).
...
| Need | Source |
|---|---|
| Page shell, nav, footer | Custom, tokens above (`docs/index.html` is the seed; restyle to this contract) |
...
- Optional: simple monochrome MBSE-adjacent diagram only if it earns space; default is typography + list  
...
**Pilot composition order:** nav → hero (claim + one primary CTA) → evidence strip (version, last sweep, entry count, text-only family pointer) → section index (primary; links to live README anchors, not a full entry mirror) → short curation note → contribute links → footer (CC0, licence enquiry, maintainer).
...
- styles/minimal-technical character (via gold sample tokens)
- patterns/progressive-disclosure and homepage-composition (via gold sample)
- README.md Contents and RELEASE-INFO.txt (product truth for this hub)
```
